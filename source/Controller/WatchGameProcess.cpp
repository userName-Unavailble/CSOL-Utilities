...
void CController::WatchGameProcess() noexcept
{
    ...
    while (true)
    {
        ...
        if (hCSOBannerProcess)
        {
            ...
            CConsole::Log(CONSOLE_LOG_LEVEL::CLL_MESSAGE, "结束 CSOBanner");
        ...
        if (game_process_state == GAME_PROCESS_STATE::GPS_BEING_CREATED)
        {
            ...
            {
                CConsole::Log(CONSOLE_LOG_LEVEL::CLL_ERROR, "获取反恐精英 Online 进程标识符时发生错误。错误代码：%lu。",
                              GetLastError());
                break;
            }
        ...
        {                                           /* 游戏进程退出，重启游戏 */
            ...
            {
                CConsole::Log(CONSOLE_LOG_LEVEL::CLL_WARNING,
        ...
        { /* 游戏进程状态未确定，确定游戏进程状态 */
            ...
            CConsole::Log(CONSOLE_LOG_LEVEL::CLL_MESSAGE, "成功获取游戏进程信息。游戏进程标识符：%lu。",
                          dwGameProcessId);
