...
void CController::RunInstance()
{
    ...
    if (!m_ExitThreads) { /* 退出命令未下达，则说明是异常退出 */
...
/* 开始运行时为 0 模式，两个 watcher 线程和一个 dispatcher 线程都处于阻塞状态，需要将 finished 置位，否则会导致热键处理线程死锁 */
m_GameProcessWatcherFinished(true),
m_InGameStateWatcherFinished(true),
m_FixedCommandDispatcherFinished(true),
