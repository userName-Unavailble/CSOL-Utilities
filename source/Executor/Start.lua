...
local function valid()
    ...
    return math.abs(current_time - CmdTime) < 2 and not Runtime:is_paused() -- 命令未失效、未即时暂停
end
...
function Start()
    ...
    do
        ...
        if (Cmd == Command.CMD_START_GAME_ROOM and valid()) -- 开始游戏
        then
            Executor:start_game_room()
        elseif (Cmd == Command.CMD_CHOOSE_CLASS and valid()) -- 选定角色
        then
            Executor:choose_class()
        elseif (Cmd == Command.CMD_PLAY_GAME_NORMAL and valid()) -- 24H 挂机模式（常规）
        then
            Executor:try_confirm()
            Player:buy_part_weapon(PartWeaponList)
            Player:play(DefaultWeaponList)
        elseif (Cmd == Command.CMD_PLAY_GAME_EXTEND and valid()) -- 24H 挂机模式（扩展）
        then
            Executor:try_confirm()
            Player:use_special_weapon(SpecialWeapon)
            Player:play(ExtendedWeaponList)
        elseif (Cmd == Command.CMD_CREATE_ROOM and valid()) -- 创建房间功能
        then
            Executor:create_game_room()
        elseif (Cmd == Command.CMD_COMBINE_PARTS and valid()) -- 合成配件功能
        then
            Executor:combine_parts()
        elseif (Cmd == Command.CMD_PURCHASE_ITEM and valid()) -- 购买物品功能
        then
        ...
        elseif (Cmd == Command.CMD_LOCATE_CURSOR and valid()) -- 光标定位功能
        then
