...
\textbf{\color{red}v1.3.14 引入了创建房间时的锁房功能，锁房可防止有人进入房间恶意举报。}
...
如果您不需要设置密码，则将 \lstinline{ROOM_USE_CUSTOM_PASSWORD} 设置为 \lstinline{true} 后，将房间自定义密码像下面这样设置为空字符串（引号内没有任何内容）即可。

\begin{minted}[breakautoindent, breaklines]{lua}
    ROOM_CUSTOM_PASSWORD = ""
\end{minted}

\textbf{\color{red} v1.3.15 中加入了自动使用角色技能的功能，配置角色的技能冷却时间即可使用。}
...
打开 \lstinline{WeaponList.lua} 文件。文件中已经提供了大量样例，根据下述的说明仔细修改即可。对于有一定编程经验的用户，阅读完本节内容后可以参阅\nameref{section_advanced_usage}了解更多自定义使用方法。
...
\begin{figure}
    \Centering
    \parbox[l]{\textwidth}{v1.3.15 提供了使用万钧神威挂机的武器对象（\lstinline{WeaponList.lua} 样例文件中），您只需要配置购买序列即可正常使用万钧神威进行挂机（图 \ref{ch2fig-use-brionac}，使用方式为两 R 一旋风）。}
    \includegraphics[width=\textwidth]{docs/assets/brionac.png}
    \caption{万钧神威武器对象，修改武器购买序列即可正常使用}
    \label{ch2fig-use-brionac}
\end{figure}
